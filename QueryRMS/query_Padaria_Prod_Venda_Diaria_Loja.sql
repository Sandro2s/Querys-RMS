...
-- Descrição : Movimento diario da padaria dos produtos indicado.Traz o produto vendido e entrada por produção
--             da loja.
...
select 
       ...
       to_number(ag.eschljc_codigo || ag.eschljc_digito, '999') as Loja,
  ...
  from aa3citem  
     where git_secao = 306 --in (108,112,301,302,303,304,306)
        and git_grupo = 8
     /* and git_cod_item||git_digito in (2151,2160,2453,3255,3271,15644,16144,17396,17620,17710,
17736,17833,18023,19577,22390,22888,22926,23680,23744,
23922,23965,24023,24040,24759,31658,37583,37591,37605,
37621,37630,37648,37656,37672,38032,38784,38792,38814,
38822,39004,39241,39250,39276,39292,39314,39624,40550,
40975,40983,41009,43290,43311,43427,43435,43737,49522,
49948,62219,72290,3156559
        )*/)aa
 ...
 where ag.eschc_data between 1||to_char(to_date('&Dta_Inicial','dd/mm/yyyy'),'yymmdd') and 1||to_char(to_date('&Dta_Final','dd/mm/yyyy'),'yymmdd')--&Dta_Ini and &Dta_Fin --1100501 and 1100531
   ...
   and ag.eschljc_codigo in (select tip_codigo from aa2ctipo,aa2cloja where tip_codigo = loj_codigo and tip_loj_cli = 'L'and tip_natureza = 'LS'and loj_data_fecha = 0)
      -- (1, 2, 3, 4, 5, 7, 8,11, 12, 15, 18, 19, 20, 21, 22, 25, 26, 29)
...
Union all
select to_number(ag.esclc_codigo || ag.esclc_digito, '999') as Loja,
       to_char(ag.esitc_codigo ||'-'|| ag.esitc_digito) as Codigo,
       aa.descricao as Descricao
       ,ag.eschc_agenda as agenda
       ,case when to_char (substr(ag.eschc_data,6)) = '01' then sum(ag.entsaic_quanti_un)  end as A
  ...
  from aa3citem  
     where git_secao = 306 --in (108,112,301,302,303,304,306)
       and git_grupo = 8
   /*   git_cod_item||git_digito in (2151,2160,2453,3255,3271,15644,16144,17396,17620,17710,
17736,17833,18023,19577,22390,22888,22926,23680,23744,
23922,23965,24023,24040,24759,31658,37583,37591,37605,
37621,37630,37648,37656,37672,38032,38784,38792,38814,
38822,39004,39241,39250,39276,39292,39314,39624,40550,
